...
en la representación de filas-ancla del Paso 1D:
    ...
    theta    : 1 valor    -> ángulo (auxiliar, para el prior posicional del Paso 5)

Las cabezas se aplican a la salida de TODAS las capas del decoder (para pérdidas auxiliares
en el Paso 4); en inferencia se usa la última capa.
"""
...
def decode_lanes(pred: dict, layer: int = -1, conf_thresh: float | None = 0.5,
    ...
    Si `conf_thresh` es None, devuelve las queries todas (útil para visualizar candidatos).
    """
