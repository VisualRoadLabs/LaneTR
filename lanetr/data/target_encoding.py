"""Codificación de carriles a la representación de FILAS-ANCLA del modelo (Paso 1D).
...
transformer predecirá exactamente esto, y la LaneIoU se calcula fila a fila sobre ello.
...
Por carril codificamos:
    ...
    theta   : ángulo (grados) de la dirección cercano->lejano (para el prior posicional).
    slot    : 0..3 si se conoce.

`R = num_rows` (por defecto 144; configurable para ablations 72 vs 144).
Las filas van de y=0 (lejos, arriba) a y=img_h-1 (cerca, abajo).
"""
...
def reprojection_error(points: np.ndarray, target: LaneTarget, row_ys: np.ndarray,
                       img_w=IMG_W_DEFAULT) -> tuple[float, float]:
    """Error horizontal (px) entre la polilínea original y la decodificada, evaluado en
    los `y` de los puntos originales que caen dentro del tramo decodificado."""
    dec = decode_lane(target.xs, target.valid, row_ys, img_w)
    if len(dec) < 2:
        return 0.0, 0.0
    dy, dx = dec[:, 1], dec[:, 0]
    o = np.argsort(dy)
    oy, ox = points[:, 1], points[:, 0]
    m = (oy >= dy.min()) & (oy <= dy.max())
    if not m.any():
        return 0.0, 0.0
    interp_x = np.interp(oy[m], dy[o], dx[o])
    err = np.abs(interp_x - ox[m])
    return float(err.mean()), float(err.max())


def encode_sample(lanes: list[np.ndarray], slots: list, row_ys: np.ndarray,
