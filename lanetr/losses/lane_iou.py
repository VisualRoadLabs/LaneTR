"""LaneIoU diferenciable, sensible al ángulo (Paso 4.1) — la aportación de la tesis.

Port fiel de CLRerNet (`LaneIoULoss` / `LaneIoUCost`) adaptado a NUESTRA representación de
filas-ancla (Paso 1D): los carriles son `xs` (R,) en x normalizada [0,1] con una máscara
booleana `valid` (R,) explícita, en vez del centinela x∉[0,1] de CLRerNet.

Idea (vs. la LineIoU de CLRNet):
  - LineIoU: a cada fila le da una anchura virtual CONSTANTE `lane_width`.
  - LaneIoU: la anchura se adapta al ángulo local del carril:
        w_i = lane_width · √(Δx_i² + Δy_i²) / Δy_i  =  lane_width / cos(θ_i)
    En tramos verticales w_i ≈ lane_width; en tramos inclinados se ensancha, igual que la
    banda perpendicular de 30 px de la métrica de evaluación. Por eso LaneIoU correlaciona
    mejor con la métrica F1.
...
# Mitades de anchura virtual por defecto (normalizadas por 800), como en CLRerNet.
LANE_WIDTH = 7.5 / 800   # LaneIoU (se ensancha con el ángulo)
LINE_WIDTH = 15.0 / 800  # LineIoU (constante)
...
def _halfwidths(pred_xs, gt_xs, angle_aware, lane_width, img_w, img_h):
    """Anchuras (pred, gt). La de pred se calcula DETACHED (sin gradiente por la anchura)."""
    if angle_aware:
        pw = _angle_halfwidth(pred_xs.detach(), lane_width, img_w, img_h)
        tw = _angle_halfwidth(gt_xs, lane_width, img_w, img_h)
    else:
        pw = torch.full_like(pred_xs, lane_width)
        tw = torch.full_like(gt_xs, lane_width)
    return pw, tw
...
# --------------------------------------------------------------------------- #
def _iou_pairwise(pred_xs, gt_xs, gt_valid, angle_aware, lane_width, img_w, img_h):
    pw, tw = _halfwidths(pred_xs, gt_xs, angle_aware, lane_width, img_w, img_h)
...
def lane_iou_pairwise(pred_xs, gt_xs, gt_valid, lane_width=LANE_WIDTH, img_w=IMG_W, img_h=IMG_H):
    ...
    return _iou_pairwise(pred_xs, gt_xs, gt_valid, True, lane_width, img_w, img_h)


def line_iou_pairwise(pred_xs, gt_xs, gt_valid, lane_width=LINE_WIDTH):
    """Matriz LineIoU (P×G) con anchura constante (CLRNet) — para la ablation."""
    return _iou_pairwise(pred_xs, gt_xs, gt_valid, False, lane_width, IMG_W, IMG_H)


# --------------------------------------------------------------------------- #
# Versión por pares ya emparejados (N) — para la pérdida
# --------------------------------------------------------------------------- #
def _iou_matched(pred_xs, gt_xs, gt_valid, angle_aware, lane_width, img_w, img_h):
    pw, tw = _halfwidths(pred_xs, gt_xs, angle_aware, lane_width, img_w, img_h)
...
def lane_iou_loss(pred_xs, gt_xs, gt_valid, lane_width=LANE_WIDTH, img_w=IMG_W, img_h=IMG_H,
    ...
    iou = _iou_matched(pred_xs, gt_xs, gt_valid, True, lane_width, img_w, img_h)
    loss = 1.0 - iou
    if reduction == "mean":
        return loss.mean()
    if reduction == "sum":
...
def line_iou_loss(pred_xs, gt_xs, gt_valid, lane_width=LINE_WIDTH, reduction="mean"):
    """Pérdida LineIoU (anchura constante) — para la ablation."""
    iou = _iou_matched(pred_xs, gt_xs, gt_valid, False, lane_width, IMG_W, IMG_H)
    loss = 1.0 - iou
    if reduction == "mean":
        return loss.mean()
    if reduction == "sum":
...
def lane_iou_value(pred_xs, gt_xs, gt_valid, angle_aware=True, lane_width=None,
                   img_w=IMG_W, img_h=IMG_H):
    """IoU por par (N,) sin reducir (utilidad para análisis/figuras)."""
    if lane_width is None:
        lane_width = LANE_WIDTH if angle_aware else LINE_WIDTH
    return _iou_matched(pred_xs, gt_xs, gt_valid, angle_aware, lane_width, img_w, img_h)
