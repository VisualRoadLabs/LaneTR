...
def main() -> int:
    ...
    print(f"Deformable: {n_heads} cabezas × 3 niveles × {n_pts} puntos = "
          f"{n_heads*3*n_pts} muestras/query  (vs 5250 tokens de la atención densa)")
    ...
    for c, q in enumerate(SHOW):
        pts = sl[q].reshape(-1, 2).cpu().numpy()          # (n_heads*n_levels*n_points, 2)
        ax.scatter(pts[:, 0] * IMG_W, pts[:, 1] * IMG_H, s=12, color=pal[c], alpha=0.7,
                   label=f"query {q}")
        rx, ry = ref[q].cpu().numpy()
        ax.scatter([rx * IMG_W], [ry * IMG_H], marker="*", s=220, color=pal[c],
